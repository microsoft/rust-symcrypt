...
//! ```
//!
use crate::ecc::{curve_to_num_format, EcKey};
...
impl EcKey {
    ...
    ///
    /// If the key usage is not [`EcKeyUsage::EcDhAndEcDsa`], the function will return a [`SymCryptError::InvalidArgument`].
...
mod test {
    ...
    fn test_ecdh_nist_p256() {
        let ecdh_1_private =
            EcKey::generate_key_pair(CurveType::NistP256, EcKeyUsage::EcDh).unwrap();
        let ecdh_2_private =
            EcKey::generate_key_pair(CurveType::NistP256, EcKeyUsage::EcDh).unwrap();
    ...
    fn test_ecdh_nist_p384() {
        let ecdh_1_private =
            EcKey::generate_key_pair(CurveType::NistP384, EcKeyUsage::EcDh).unwrap();
        let ecdh_2_private =
            EcKey::generate_key_pair(CurveType::NistP384, EcKeyUsage::EcDh).unwrap();
    ...
    fn test_ecdh_wrong_usage() {
        let ecdh_1_private =
            EcKey::generate_key_pair(CurveType::Curve25519, EcKeyUsage::EcDsa).unwrap();
        let ecdh_2_private =
            EcKey::generate_key_pair(CurveType::Curve25519, EcKeyUsage::EcDsa).unwrap();
        ...
        let secret_agreement_1 = ecdh_1_private
            .ecdh_secret_agreement(ecdh_2_public)
            .unwrap_err();
    ...
    fn test_ecdh_no_private_key_set() {
        ...
        let secret_agreement_1 = ecdh_1_no_private_key
            .ecdh_secret_agreement(ecdh_2_public)
            .unwrap_err();
